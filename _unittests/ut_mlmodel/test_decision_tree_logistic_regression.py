...
class TestDecisionTreeLogisticRegression(ExtTestCase):
    ...
    def test_pandas(self):
        ...
        Y = numpy.array([0, 1])
        clq = DecisionTreeLogisticRegression()
        clq.fit(X, Y)
        pred2 = clq.predict(X)
    ...
    def test_classifier_pickle(self):
        ...
        test_sklearn_pickle(lambda: DecisionTreeLogisticRegression(), X, Y)

    def test_classifier_clone(self):
        test_sklearn_clone(lambda: DecisionTreeLogisticRegression())
    ...
    def test_classifier_grid_search(self):
        ...
        self.assertRaise(lambda: test_sklearn_grid_search_cv(
            lambda: DecisionTreeLogisticRegression(), X, Y), ValueError)
        res = test_sklearn_grid_search_cv(lambda: DecisionTreeLogisticRegression(),
                                          X, Y, max_depth=[2, 3])
