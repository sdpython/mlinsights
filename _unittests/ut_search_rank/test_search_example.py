...
from src.mlinsights.search_rank import SearchEngineExamples


class TestSearchExamples(ExtTestCase):
    ...
    def test_search_examples(self):
        ...
        se = SearchEngineExamples(n_neighbors=5)
        r = repr(se)
        self.assertEqual(r.replace("\n", "").replace(" ", ""),
                         'SearchEngineExamples(n_neighbors=5)')
